...
const SIMPLE: Assumptions = {
  ...
  obligations: [
    { id: 'a', label: 'Support', monthlyAmount: 500, startMonth: 1, endMonth: 24 },
  ],
...
describe('housingBudget', () => {
  ...
  it('counts the co-resident, because that money only exists once you own', () => {
    const alone: Assumptions = { ...SIMPLE, coResident: { ...SIMPLE.coResident, enabled: false } };
    expect(housingBudget(alone, { atMonth: 1, reserveForSavings: 0 }).monthlyBudget).toBe(5_500);
  });
  ...
  it('excludes rent, since that is the thing being replaced', () => {
    ...
    expect(housingBudget(dearerRent, { atMonth: 1, reserveForSavings: 0 }).monthlyBudget).toBe(
      6_300,
    );
  });
  ...
  it('drops obligations that have ended by the month asked about', () => {
    ...
    expect(housingBudget(SIMPLE, { atMonth: 25, reserveForSavings: 0 }).monthlyBudget).toBe(6_800);
  });

  it('holds back whatever you reserve for saving', () => {
    expect(housingBudget(SIMPLE, { atMonth: 1, reserveForSavings: 800 }).monthlyBudget).toBe(5_500);
  });
  ...
  it('shows its working', () => {
    ...
    const { breakdown: d } = b;
    expect(
      d.income + d.coResident - d.livingCosts - d.obligations - d.retirementContributions - d.reserveForSavings,
    ).toBe(b.monthlyBudget);
  });
});

describe('maxAffordablePrice', () => {
  const args = { effectiveTaxRate: 0.0109, insuranceMonthly: 150 };
  ...
  it('produces a price whose all-in cost matches the budget', () => {
    ...
    for (const budget of [2_000, 3_000, 4_500, 6_300]) {
      const price = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: budget });
      const cost = monthlyCostOfHouse(SIMPLE, { ...args, price });
      expect(cost.total).toBeCloseTo(budget, 6);
    }
  });

  it('buys less house where the tax rate is higher', () => {
    const cheapTax = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 3_000, effectiveTaxRate: 0.009 });
    const dearTax = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 3_000, effectiveTaxRate: 0.03 });
    ...
    expect(expectedRatio).toBeLessThan(0.80);
  });

  it('buys less house at a higher mortgage rate', () => {
    const dearer: Assumptions = { ...SIMPLE, home: { ...SIMPLE.home, mortgageRateAnnual: 0.08 } };
    expect(maxAffordablePrice(dearer, { ...args, monthlyBudget: 3_000 })).toBeLessThan(
      maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 3_000 }),
    );
  });
  ...
  it('buys MORE house on a small deposit, because the payment is what binds', () => {
    ...
    const small: Assumptions = { ...SIMPLE, home: { ...SIMPLE.home, downPaymentPct: 0.03 } };
    const bigDeposit = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 3_000 });
    const smallDeposit = maxAffordablePrice(small, { ...args, monthlyBudget: 3_000 });
  ...
  it('charges no PMI at a 20% deposit', () => {
    const cost = monthlyCostOfHouse(SIMPLE, { ...args, price: 400_000 });
    expect(cost.pmi).toBe(0);
  });

  it('charges PMI below the threshold', () => {
    const small: Assumptions = { ...SIMPLE, home: { ...SIMPLE.home, downPaymentPct: 0.03 } };
    const cost = monthlyCostOfHouse(small, { ...args, price: 400_000 });
    expect(cost.pmi).toBeCloseTo((400_000 * 0.97 * 0.0055) / 12, 6);
  });

  it('returns zero rather than a negative price on an impossible budget', () => {
    expect(maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 0 })).toBe(0);
    expect(maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: -500 })).toBe(0);
  });

  it('rises roughly in line with the budget', () => {
    const a = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 2_150 });
    const b = maxAffordablePrice(SIMPLE, { ...args, monthlyBudget: 4_150 });
...
describe('monthlyCostOfHouse', () => {
  const args = { effectiveTaxRate: 0.0109, insuranceMonthly: 150, price: 400_000 };

  it('totals its own components', () => {
    const c = monthlyCostOfHouse(SIMPLE, args);
  ...
  it('charges tax as the effective rate on the price', () => {
    expect(monthlyCostOfHouse(SIMPLE, args).tax).toBeCloseTo((400_000 * 0.0109) / 12, 6);
  });

  it('accrues upkeep at the maintenance rate', () => {
    expect(monthlyCostOfHouse(SIMPLE, args).maintenance).toBeCloseTo((400_000 * 0.01) / 12, 6);
  });
});
...
describe('cashToClose', () => {
  ...
  it('adds the upfront premium on a small deposit', () => {
    ...
    expect(cashToClose(fha, 400_000)).toBeCloseTo(400_000 * (0.03 + cc + 0.97 * 0.0175), 6);
  });
  ...
  it('respects a dollar cap on the assistance', () => {
    ...
    expect(cashToClose(capped, 400_000)).toBeCloseTo(cashToClose(SIMPLE, 400_000) - 6_000, 6);
  });

  it('never goes below zero, however generous the assistance', () => {
    const enormous: Assumptions = {
      ...SIMPLE,
      home: { ...SIMPLE.home, assistanceEnabled: true, assistancePctOfPrice: 0.9, assistanceMaxAmount: null, assistanceRepayment: 'forgiven' },
    };
...
describe('affordabilityTimeline -- is it worth waiting?', () => {
  const cashTrack = Array.from({ length: 120 }, (_, i) => 20_000 + i * 500);
  const common = {
  ...
  it('returns one point per month', () => {
    const t = affordabilityTimeline(SIMPLE, { ...common, medianPriceToday: 250_000 });
  ...
  it('appreciates the house while you wait', () => {
    ...
    const t = affordabilityTimeline(appreciating, { ...common, medianPriceToday: 250_000 });
  ...
  it('grows what you can carry as your pay rises', () => {
    ...
    const t = affordabilityTimeline(raising, { ...common, medianPriceToday: 250_000 });
  ...
  it('steps the budget up sharply when a commitment ends', () => {
    ...
    const frozenPay: Assumptions = { ...SIMPLE, income: { ...SIMPLE.income, growthAnnual: 0 } };
    const t = affordabilityTimeline(frozenPay, { ...common, medianPriceToday: 250_000 });
...
describe('waitingVerdict', () => {
  const cashTrack = Array.from({ length: 180 }, (_, i) => 20_000 + i * 500);
  const common = {
    effectiveTaxRate: 0.016,
    insuranceMonthly: 150,
    months: 180,
  ...
  it('separates when the payment becomes carryable from when the cash arrives', () => {
    ...
    if (v.affordableFrom !== null) {
      expect(v.affordableFrom).toBe(Math.max(v.monthlyGapClosesAt ?? 0, v.cashReadyAt ?? 0));
    }
  });

  it('says never when the house stays out of reach for the whole window', () => {
    const t = affordabilityTimeline(SIMPLE, { ...common, medianPriceToday: 5_000_000 });
  ...
  it('calls the gap widening when houses outrun your income', () => {
    ...
    const t = affordabilityTimeline(losing, { ...common, medianPriceToday: 600_000 });
  ...
  it('calls it closing when your income outruns the houses', () => {
    ...
    const t = affordabilityTimeline(winning, { ...common, medianPriceToday: 600_000 });
