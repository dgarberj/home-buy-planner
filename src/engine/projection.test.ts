...
const BUY_M12: ScenarioConfig = { ...RENT_FOREVER, id: 'buy12', name: 'Buy at 12', buyMonth: 12 };
...
describe('runProjection -- baseline month with all rates at zero', () => {
  ...
  it('nets worth to savings plus retirement while renting', () => {
    ...
    expect(m12.netWorth).toBeCloseTo(m12.liquidSavings + m12.retirementBalance, 6);
...
describe('runProjection -- the buy-month transition', () => {
  ...
  it('builds equity every month after the purchase', () => {
    for (let m = 13; m <= 60; m++) {
      expect(at(rows, m).homeEquity).toBeGreaterThan(at(rows, m - 1).homeEquity);
    }
  });
  ...
  it('leaves months before the purchase identical to never buying', () => {
    ...
    for (let m = 1; m <= 11; m++) {
      expect(at(rows, m).liquidSavings).toBeCloseTo(at(renting, m).liquidSavings, 9);
...
describe('runProjection -- job loss', () => {
  const scenario: ScenarioConfig = { ...RENT_FOREVER, id: 'jl', hasJobLoss: true };
  ...
  it('burns cash at the hand-computed rate during the gap', () => {
    ...
    for (let m = 13; m <= 18; m++) expect(at(rows, m).netCashFlow).toBeCloseTo(-2_000, 6);
  ...
  it('applies nothing when the duration is zero', () => {
    const zero: Assumptions = { ...FLAT, jobLoss: { ...FLAT.jobLoss, durationMonths: 0 } };
    expect(runProjection(zero, scenario, 60).some((r) => r.jobLossActive)).toBe(false);
  });
});
...
describe('runProjection -- growth, inflation and returns', () => {
  ...
  it('inflates expenses and rent together by exactly the annual rate', () => {
    const a: Assumptions = { ...FLAT, expenses: { ...FLAT.expenses, inflationAnnual: 0.04 } };
    const rows = runProjection(a, RENT_FOREVER, 60);
  ...
  it('compounds savings and retirement returns monthly', () => {
    ...
    expect(at(rows, 1).liquidSavings).toBeCloseTo(100_000 * Math.pow(1.12, 1 / 12) + 2_000, 6);
...
describe('home price, appreciation and the cost of waiting', () => {
  ...
  it('makes buying later cost more up front and carry a bigger loan', () => {
    ...
    expect(at(late, 25).purchaseOutflow).toBeGreaterThan(at(early, 13).purchaseOutflow);
    expect(at(late, 25).housingPayment).toBeGreaterThan(at(early, 13).housingPayment);
...
describe('negative cash is surfaced, not hidden', () => {
  it('lets liquid savings go below zero when the purchase is unaffordable', () => {
    const broke: Assumptions = { ...FLAT, savings: { ...FLAT.savings, cashBalance: 20_000 } };
    ...
    expect(at(rows, 12).liquidSavings).toBeCloseTo(42_000 + 1_281.4393 - 92_000, 2);
  });
});
...
describe('the cash buffer and the investment sweep', () => {
  ...
  it('keeps everything in cash while the balance is under the target', () => {
    const lean: Assumptions = { ...FLAT, savings: { ...FLAT.savings, cashBalance: 10_000 } };
  ...
  it('always reports liquid savings as the two pools added together', () => {
    ...
    for (const row of rows) {
      expect(row.liquidSavings).toBeCloseTo(row.cashBalance + row.investmentBalance, 9);
    }
  });
  ...
  it('ends up ahead when investments outperform cash', () => {
    ...
    expect(at(grown, 120).liquidSavings).toBeGreaterThan(at(flat, 120).liquidSavings);
    // ...and the gain sits in the invested pool, not the buffer.
    expect(at(grown, 120).cashBalance).toBeCloseTo(at(flat, 120).cashBalance, 6);
  });
});
...
describe('retirement contributions over a long horizon', () => {
  const raising: Assumptions = {
    ...FLAT,
    income: { monthlyTakeHome: 10_000, growthAnnual: 0.03, annualBonusNet: 0, annualBonusMonth: 1, calendarStartMonth: 1 },
  };
  ...
  it('holds contributions flat when the flag is off', () => {
    ...
    expect(at(rows, 13).retirementBalance - at(rows, 12).retirementBalance).toBeCloseTo(1_500, 6);
    expect(at(rows, 60).retirementBalance - at(rows, 59).retirementBalance).toBeCloseTo(1_500, 6);
  });
  ...
  it('grows contributions in step with pay when the flag is on', () => {
    ...
    expect(at(rows, 13).retirementBalance - at(rows, 12).retirementBalance).toBeCloseTo(1_545, 6);
    // Two years: 1,500 * 1.03^2.
    expect(at(rows, 25).retirementBalance - at(rows, 24).retirementBalance).toBeCloseTo(
      1_500 * 1.03 * 1.03,
      6,
    );
  });
  ...
  it('still pauses growing contributions during a job loss', () => {
    ...
    expect(at(rows, 14).retirementBalance).toBeCloseTo(at(rows, 13).retirementBalance, 6);
  });
});
...
describe('ages and retirement milestones', () => {
  ...
  it('breaks each milestone down by where the money sits', () => {
    ...
    expect(s.retirementAtAge[42]).toBeCloseTo(at(rows, 25).retirementBalance, 9);
    expect(s.homeEquityAtAge[42]).toBeCloseTo(at(rows, 25).homeEquity, 9);
    expect(s.investmentsAtAge[42]).toBeCloseTo(at(rows, 25).investmentBalance, 9);
  });
...
describe('home upkeep', () => {
  ...
  it('starts accruing the month you buy', () => {
    ...
    expect(at(rows, 12).homeMaintenance).toBeCloseTo(400_000 * UPKEEP / 12, 6);
  });
  ...
  it('still comes out of cash flow', () => {
    ...
    expect(at(rows, 12).liquidSavings).toBeCloseTo(at(without, 12).liquidSavings - 333.3333, 3);
  });
  ...
  it('grows as the house appreciates', () => {
    ...
    const rows = runProjection(appreciating, { ...BUY_M12, buyMonth: 13 }, 60);
    expect(at(rows, 13).homeMaintenance).toBeCloseTo((412_000 * UPKEEP) / 12, 6);
    expect(at(rows, 25).homeMaintenance).toBeCloseTo((412_000 * 1.03 * UPKEEP) / 12, 6);
  });
  ...
  it('raises the emergency fund target for owners', () => {
    ...
    const target = 6 * (row.totalExpenses + row.housingPayment + row.homeMaintenance);
    ...
    expect(target).toBeGreaterThan(6 * (without.totalExpenses + without.housingPayment));
  });
  ...
  it('slows the refill of the buffer after a purchase drains it', () => {
    ...
    expect(row.investmentBalance).toBeCloseTo(at(rows, 13).investmentBalance, 6);
  });
  ...
  it('totals across the horizon', () => {
    ...
    expect(s.totalMaintenancePaid).toBeCloseTo(49 * (400_000 * UPKEEP) / 12, 4);
  });
...
describe('mortgage insurance', () => {
  ...
  it('is never charged on a 20% down payment', () => {
    const rows = runProjection({ ...FLAT, home: { ...FLAT.home, pmiAnnualPct: 0.006 } }, BUY_M12, 60);
    expect(rows.every((r) => r.pmiPayment === 0)).toBe(true);
    const s = summarizeScenario({ ...FLAT, home: { ...FLAT.home, pmiAnnualPct: 0.006 } }, BUY_M12, 60);
  ...
  it('falls away exactly when the loan-to-value ratio clears the threshold', () => {
    ...
    expect(at(rows, endsAt).mortgageBalance / at(rows, endsAt).homeValue).toBeLessThan(0.8 + 1e-9);
    // ...and the month before, it was still above it.
    expect(at(rows, endsAt - 1).mortgageBalance / at(rows, endsAt - 1).homeValue).toBeGreaterThan(0.8);
...
describe('fixed obligations', () => {
  const withSupport: Assumptions = {
    ...FLAT,
    obligations: [
      { id: 'o1', label: 'Support (both)', monthlyAmount: 500, startMonth: 1, endMonth: 24 },
      { id: 'o2', label: 'Support (one)', monthlyAmount: 300, startMonth: 25, endMonth: 36 },
    ],
  };
  ...
  it('is NOT cut during a job loss, unlike ordinary expenses', () => {
    const rows = runProjection(withSupport, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    const m13 = at(rows, 13);
  ...
  it('makes a job loss meaningfully harder to survive', () => {
    const without = summarizeScenario(FLAT, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    const withIt = summarizeScenario(withSupport, { ...RENT_FOREVER, hasJobLoss: true }, 60);
  ...
  it('counts towards the emergency fund target', () => {
    ...
    expect(row.cashBalance).toBeCloseTo(
      6 * (row.totalExpenses + row.obligations + row.housingPayment + row.homeMaintenance),
      6,
    );
  });

  it('pushes out the month the house becomes affordable', () => {
    const saving: Assumptions = { ...FLAT, savings: { ...FLAT.savings, cashBalance: 50_000 } };
    const savingWithSupport: Assumptions = { ...saving, obligations: withSupport.obligations };
  ...
  it('runs to the horizon when no end month is given', () => {
    const openEnded: Assumptions = {
      ...FLAT,
      obligations: [
        { id: 'o1', label: 'Forever', monthlyAmount: 100, startMonth: 1, endMonth: null },
      ],
    };
...
describe('a down payment below the threshold', () => {
  const lowDown: Assumptions = {
    ...FLAT,
    home: { ...FLAT.home, downPaymentPct: 0.1, pmiAnnualPct: 0.006, pmiUpfrontPct: 0.0175 },
  };

  it('adds the upfront premium to the cash needed at closing', () => {
    // 10% of 400,000 down + 3% closing + 1.75% of the 360,000 loan
    expect(cashRequiredToBuy(lowDown, 1)).toBeCloseTo(40_000 + 12_000 + 6_300, 4);
  });
  ...
  it('charges nothing upfront on a 20% down payment', () => {
    const bigDown: Assumptions = { ...FLAT, home: { ...FLAT.home, pmiUpfrontPct: 0.0175 } };
    expect(cashRequiredToBuy(bigDown, 1)).toBeCloseTo(92_000, 4);
    expect(at(runProjection(bigDown, BUY_M12, 60), 12).purchaseOutflow).toBeCloseTo(92_000, 4);
  });

  it('makes the house take longer to afford than the headline deposit suggests', () => {
    const saving = { ...FLAT, savings: { ...FLAT.savings, cashBalance: 20_000 } };
    const cheapDeposit = summarizeScenario({ ...saving, home: lowDown.home }, RENT_FOREVER, 120);
...
describe('retirement contributions -- who puts in what, and when', () => {
  ...
  const withLump: Assumptions = {
    ...FLAT,
    retirement: { ...FLAT.retirement, employerAnnualLump: 3_000, employerAnnualLumpMonth: 1 },
  };
  ...
  it('grows the balance by both contributions together', () => {
    const rows = runProjection(FLAT, RENT_FOREVER, 12);
    const growth = at(rows, 2).retirementBalance - at(rows, 1).retirementBalance;
  ...
  it('never lets the employer lump touch your cash flow', () => {
    ...
    for (let m = 1; m <= 24; m++) {
      expect(at(withIt, m).netCashFlow).toBeCloseTo(at(withOut, m).netCashFlow, 9);
      expect(at(withIt, m).liquidSavings).toBeCloseTo(at(withOut, m).liquidSavings, 9);
    }
    // ...but it does grow retirement, by exactly two years of lumps.
    expect(at(withIt, 24).retirementBalance - at(withOut, 24).retirementBalance).toBeCloseTo(
      6_000,
      6,
    );
  });
  ...
  it('adds up to the right total over a year', () => {
    ...
    expect(at(rows, 12).retirementBalance - 100_000).toBeCloseTo(employee + employer, 6);
  });
  ...
  it('pauses every stream during a job loss, lump included', () => {
    ...
    const rows = runProjection(januaryLoss, { ...RENT_FOREVER, hasJobLoss: true }, 24);
    ...
    expect(at(rows, 13).retirementBalance).toBeCloseTo(at(rows, 12).retirementBalance, 6);
  });

  it('keeps paying the lump when the pause flag is off', () => {
    const noPause: Assumptions = {
      ...withLump,
      jobLoss: { ...FLAT.jobLoss, startMonth: 12, durationMonths: 6, pauseRetirementContributions: false },
    };
    const rows = runProjection(noPause, { ...RENT_FOREVER, hasJobLoss: true }, 24);
    expect(at(rows, 13).employerContribution).toBe(3_500);
  });
...
describe('the household’s actual contribution stack', () => {
  ...
  it('totals the year correctly across all four streams', () => {
    ...
    expect((yours + employer) / SALARY).toBeLessThan(0.20);
  });
});
...
describe('an annual bonus', () => {
  ...
  it('leaves the other eleven months exactly as they were', () => {
    ...
    expect(at(withIt, 2).netCashFlow).toBeCloseTo(at(withOut, 2).netCashFlow, 9);
    expect(at(withIt, 12).netCashFlow).toBeCloseTo(at(withOut, 12).netCashFlow, 9);
  });
  ...
  it('is cut during a job loss -- you are not there to earn it', () => {
    ...
    const rows = runProjection(jobLossInJanuary, { ...RENT_FOREVER, hasJobLoss: true }, 24);
  ...
  it('leaves cash thinner than the smeared version for the eleven dry months', () => {
    ...
    for (let m = 1; m <= 11; m++) {
      expect(at(lumpy, m).liquidSavings).toBeLessThan(at(smooth, m).liquidSavings);
    }
    // The bonus lands in month 12 and the gap closes.
    expect(at(lumpy, 12).bonusIncome).toBe(6_000);
    expect(at(lumpy, 12).liquidSavings).toBeGreaterThan(at(smooth, 11).liquidSavings);
  });
...
describe('a partner returning to work', () => {
  ...
  it('turns strongly positive the month childcare ends', () => {
    ...
    expect(at(rows, 37).netCashFlow - at(rows, 36).netCashFlow).toBeCloseTo(1_200, 6);
  ...
  it('keeps paying when the other earner loses their job', () => {
    ...
    const rows = runProjection(returning, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    expect(at(rows, 13).jobLossActive).toBe(true);
  ...
  it('is cut too when told it shares the same risk', () => {
    ...
    const rows = runProjection(sameEmployer, { ...RENT_FOREVER, hasJobLoss: true }, 60);
  ...
  it('does NOT hedge a job loss while childcare is still being paid', () => {
    ...
    const without = runProjection(FLAT, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    const withIt = runProjection(running, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    expect(at(withIt, 13).netCashFlow - at(without, 13).netCashFlow).toBeCloseTo(-200, 6);
  ...
  it('DOES hedge a job loss once childcare is behind you', () => {
    ...
    const without = runProjection(FLAT, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    const withIt = runProjection(afterCare, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    for (let m = 13; m <= 18; m++) {
      expect(at(withIt, m).netCashFlow - at(without, m).netCashFlow).toBeCloseTo(1_000, 6);
    }
    const a = summarizeScenario(afterCare, { ...RENT_FOREVER, hasJobLoss: true }, 60);
    const b = summarizeScenario(FLAT, { ...RENT_FOREVER, hasJobLoss: true }, 60);
  ...
  it('grows with pay rises when told to', () => {
    ...
    const rows = runProjection(raising, RENT_FOREVER, 60);
    expect(at(rows, 24).secondIncome).toBeCloseTo(1_000 * Math.pow(1.03, 23 / 12), 6);
  });
  ...
  it('inflates the childcare bill like any other cost', () => {
    ...
    expect(at(rows, 24).secondIncomeCosts).toBeCloseTo(1_200 * Math.pow(1.05, 23 / 12), 6);
  });

  it('runs the costs to the horizon when no end month is set', () => {
    const openEnded: Assumptions = {
      ...returning,
      secondIncome: { ...returning.secondIncome, additionalCostsEndMonth: null },
    };
    const rows = runProjection(openEnded, RENT_FOREVER, 60);
...
describe('drawing on the HSA rather than hoarding it', () => {
  ...
  const reimburse: Assumptions = {
    ...FLAT,
    retirement: { ...FLAT.retirement, hsaReimbursement: 3_000, hsaReimbursementMonth: 1 },
  };
  ...
  it('takes it straight out of the retirement balance', () => {
    ...
    expect(at(rows, 1).retirementBalance).toBeCloseTo(at(plain, 1).retirementBalance - 300, 6);
  });
  ...
  it('arrives in time to help with the cash needed at closing', () => {
    ...
    expect(at(withIt, 12).liquidSavings - at(without, 12).liquidSavings).toBeCloseTo(6_000, 6);
  });
  ...
  it('costs long-run growth, because the HSA compounds faster than cash', () => {
    const growing: Assumptions = {
      ...
      savings: { ...FLAT.savings, cashReturnAnnual: 0.04, investmentReturnAnnual: 0.04 },
    };
  ...
  it('never draws more than the HSA actually holds', () => {
    const overdrawn: Assumptions = {
      ...FLAT,
      retirement: { ...FLAT.retirement, currentBalance: 500, hsaReimbursement: 50_000 },
    };
...
describe('the HSA levers switch cleanly on and off', () => {
  ...
  it('returns to the untouched baseline with both off', () => {
    const off: Assumptions = {
      ...drawing,
      retirement: { ...drawing.retirement, hsaPayMedical: false, hsaTakeReimbursement: false },
    };
...
describe('diverting the HSA to the deposit', () => {
  const diverted: Assumptions = {
    ...FLAT,
    retirement: { ...FLAT.retirement, pauseHsaMax: true, pausedEmployeeMonthly: 400 },
  };
  ...
  it('lowers what goes into retirement each month', () => {
    ...
    expect(at(runProjection(FLAT, RENT_FOREVER, 24), 1).employeeContribution).toBe(1_000);
  });

  it('puts the difference straight into cash flow', () => {
    const rows = runProjection(diverted, RENT_FOREVER, 24);
    const plain = runProjection(FLAT, RENT_FOREVER, 24);
    expect(at(rows, 1).netCashFlow - at(plain, 1).netCashFlow).toBeCloseTo(600, 6);
  });
  ...
  it('builds the buffer faster, which is the whole point', () => {
    ...
    expect(at(a.months, 24).liquidSavings).toBeGreaterThan(at(b.months, 24).liquidSavings);
  });
  ...
  it('costs long-run retirement, which is the price', () => {
    ...
    expect(a.months[239]!.retirementBalance).toBeLessThan(b.months[239]!.retirementBalance);
  });
...
describe('down-payment assistance switches off cleanly', () => {
  ...
  it('does nothing when the toggle is off', () => {
    const off: Assumptions = { ...withKfit, home: { ...withKfit.home, assistanceEnabled: false } };
    expect(JSON.stringify(runProjection(off, BUY_M12, 24))).toBe(
      JSON.stringify(runProjection(FLAT, BUY_M12, 24)),
    );
  });
...
describe('a co-resident contributing to the household', () => {
  ...
  it('can be set to start immediately instead', () => {
    ...
    expect(at(runProjection(now, RENT_FOREVER, 60), 1).coResidentIncome).toBe(800);
  });
  ...
  it('adds straight to cash flow', () => {
    ...
    expect(at(rows, 13).netCashFlow - at(rows, 13).coResidentIncome).toBeLessThan(
      at(without, 13).netCashFlow,
    );
  ...
  it('leaves the price alone when switched off', () => {
    const off: Assumptions = { ...withMum, coResident: { ...withMum.coResident, enabled: false } };
  ...
  it('cushions a job loss, holding the house price constant', () => {
    ...
    const without = summarizeScenario(FLAT, { ...BUY_M12, hasJobLoss: true }, 60);
    const with_ = summarizeScenario(noPremium, { ...BUY_M12, hasJobLoss: true }, 60);
  ...
  it('shrinks the job-loss dip more than the same amount of salary would', () => {
    ...
    const viaCoResident = runProjection(noPremium, { ...BUY_M12, hasJobLoss: true }, 60);
    const viaSalary = runProjection(extraSalary, { ...BUY_M12, hasJobLoss: true }, 60);
    // Month 13 is inside the job-loss window.
    expect(at(viaCoResident, 13).netCashFlow).toBeGreaterThan(at(viaSalary, 13).netCashFlow);
  });
  ...
  it('can cost more than it brings in, once the price premium is counted', () => {
    ...
    const without = summarizeScenario(FLAT, { ...BUY_M12, hasJobLoss: true }, 60);
    const withPremium = summarizeScenario(withMum, { ...BUY_M12, hasJobLoss: true }, 60);
  ...
  it('pushes the deposit further away, because the house costs more', () => {
    const saving: Assumptions = { ...FLAT, savings: { ...FLAT.savings, cashBalance: 50_000 } };
    const savingWithMum: Assumptions = { ...saving, coResident: withMum.coResident };
    const a = summarizeScenario(saving, RENT_FOREVER, 120);
...
describe('summarizeScenario', () => {
  ...
  it('puts the minimum buffer at the bottom of a deep job-loss dip', () => {
    ...
    const s = summarizeScenario(severe, { ...RENT_FOREVER, hasJobLoss: true }, 60);
  ...
  it('reports the starting balance as the buffer when we only ever save', () => {
    ...
    const s = summarizeScenario(FLAT, { ...RENT_FOREVER, hasJobLoss: true }, 60);
  ...
  it('reports net worth at years 1, 3 and 5', () => {
    ...
    expect(s.netWorthAtYear[3]).toBeCloseTo(150_000 + 36 * 2_000 + 100_000 + 36 * 1_500, 6);
