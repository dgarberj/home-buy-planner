...
const dtiVerdictAt = (housing: number) =>
  debtToIncome({ ...SIMPLE, proposedHousing: housing }).verdict;

const dtiAtLimit = (limit: number) =>
...
describe("debtToIncome", () => {
  ...
  it("grades the ratio against the published limits", () => {
    expect(dtiVerdictAt(100)).toBe("comfortable"); // 16%
    expect(dtiVerdictAt(2_000)).toBe("comfortable"); // 35%
    expect(dtiVerdictAt(3_000)).toBe("workable"); // 45%
    expect(dtiVerdictAt(3_400)).toBe("tight"); // 49%
    expect(dtiVerdictAt(4_000)).toBe("declined"); // 55%
  });

  it("lands exactly on the boundaries", () => {
    expect(dtiAtLimit(DTI_LIMITS.conservative).verdict).toBe("comfortable");
    expect(dtiAtLimit(DTI_LIMITS.manual).verdict).toBe("workable");
    expect(dtiAtLimit(DTI_LIMITS.automated).verdict).toBe("tight");
  });
...
describe("maxPriceByDti", () => {
  ...
  it("produces a price whose housing payment lands on the limit", () => {
    const price = maxPriceByDti(base, { ...options, limit: DTI_LIMITS.manual });
    ...
    const r = debtToIncome({ ...options, proposedHousing: lenderHousing });
    expect(r.backEnd).toBeCloseTo(DTI_LIMITS.manual, 4);
  });

  it("allows more house at a looser limit", () => {
    const strict = maxPriceByDti(base, {
      ...options,
      limit: DTI_LIMITS.conservative,
    });
    const loose = maxPriceByDti(base, {
      ...options,
      limit: DTI_LIMITS.automated,
    });
    expect(loose).toBeGreaterThan(strict);
  });

  it("shrinks sharply as support rises", () => {
    const light = maxPriceByDti(base, { ...options, supportPaid: 0 });
    const heavy = maxPriceByDti(base, { ...options, supportPaid: 2_000 });
  ...
  it("gives a meaningful uplift when a nearly-finished loan drops out", () => {
    const counted = maxPriceByDti(base, options);
    const excluded = maxPriceByDti(base, { ...options, instalmentDebts: 0 });
  ...
  it("ignores upkeep, because a lender does", () => {
    ...
    const noUpkeep: Assumptions = {
      ...base,
      home: { ...base.home, maintenanceAnnualPct: 0 },
    };
    expect(maxPriceByDti(noUpkeep, options)).toBeCloseTo(
      maxPriceByDti(base, options),
      6,
    );
  });

  it("returns zero when the debts already exceed the limit", () => {
    expect(maxPriceByDti(base, { ...options, supportPaid: 6_000 })).toBe(0);
  });
  ...
  it("can be tighter than the household budget, which is the point", () => {
    ...
    const byLender = maxPriceByDti(base, {
      ...options,
      limit: DTI_LIMITS.conservative,
    });
