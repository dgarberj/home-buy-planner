...
import { deepMerge, migrateSaved, seedData } from './migrate';
import { SEED_ASSUMPTIONS, SEED_VERSION } from '../data/seed';
...
const V1_SAVE = {
  ...
  budget: [{ id: 'x', label: 'Old line', category: 'Food', type: 'variable', amount: 123 }],
  balances: [],
  scenarios: [
    { id: 'a', name: 'Old scenario', buyMonth: 18, hasJobLoss: false, enabled: true, color: '#123456' },
  ],
  settings: {
    horizonMonths: 60,
    startDate: '2026-01',
...
describe('migrateSaved -- the seed file is the source of truth', () => {
  it('replaces saved state written against an older seed', () => {
    // The whole point: editing seed.ts must actually show up in the app, even
    // on a machine that already has saved state.
    const stale = { ...V1_SAVE, seedVersion: 'something-older' };
    const out = migrateSaved(stale);
    expect(out.assumptions.income.monthlyTakeHome).toBe(SEED_ASSUMPTIONS.income.monthlyTakeHome);
    expect(out.assumptions.expenses.currentRentMonthly).toBe(
      SEED_ASSUMPTIONS.expenses.currentRentMonthly,
    );
...
describe('migrateSaved -- filling in a same-version payload', () => {
  ...
  it('supplies the retirement contribution-growth flag', () => {
    expect(typeof out.assumptions.retirement.contributionsGrowWithIncome).toBe('boolean');
  });
});
...
describe('migrateSaved -- junk input', () => {
  ...
  it('never hands back a shape the app cannot render', () => {
    ...
    for (const junk of [undefined, null, {}, 'x', 0, [], { assumptions: null }]) {
