...
interface Lever {
  ...
  flip: (a: Assumptions, next: boolean) => Partial<Assumptions>;
  hint: string;
}
...
const LEVERS: Lever[] = [
  {
    ...
    flip: (a, next) => ({ secondIncome: { ...a.secondIncome, enabled: next } }),
  ...
  },
  {
    ...
    flip: (a, next) => ({ coResident: { ...a.coResident, enabled: next } }),
  ...
  },
  {
    ...
    flip: (a, next) => ({ retirement: { ...a.retirement, hsaPayMedical: next } }),
  ...
  },
  {
    ...
    flip: (a, next) => ({ retirement: { ...a.retirement, hsaTakeReimbursement: next } }),
  ...
  },
  {
    ...
    flip: (a, next) => ({ retirement: { ...a.retirement, pauseHsaMax: next } }),
  ...
  },
  {
    ...
    flip: (a, next) => ({ home: { ...a.home, assistanceEnabled: next } }),
...
function measure(assumptions: Assumptions, scenarios: ScenarioConfig[], months: number, ages: number[]) {
  ...
  return {
    ...
    netWorth:
      summaries.reduce((sum, s) => sum + s.endingNetWorth, 0) / summaries.length,
  };
}
    ...
    <div className="border-b border-slate-200 bg-slate-50/95 backdrop-blur">
      <div className="mx-auto max-w-7xl px-6 py-2.5">
        ...
        <p className="mt-1.5 text-[11px] text-slate-400">
          Every figure is the effect of that lever being <strong>ON</strong>, so the sign does not
          change when you flip the switch. Green helps, red costs. Several help one and cost the
          other — that is the trade, not a mistake.
        </p>
      </div>
    </div>
  );
