...
const NUMBER_ROOM_INLINE = 'min-w-[5.5rem] pr-7';
...
function useNumericText(
  ...
  useEffect(() => {
    ...
    // Re-sync from props whenever the field is not being edited.
...
export function MoneyInput({
  ...
  const bind = useNumericText(value, (n) => String(n), parseFloat, onChange);
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        {...bind}
        className={`${base} ${
          variant === 'inline' ? `pl-5 ${NUMBER_ROOM_INLINE}` : `pl-6 ${NUMBER_ROOM}`
...
export function NumberInput({
  ...
  return (
    <input
      ...
      step={step}
      {...bind}
      className={`${variant === 'inline' ? `${INLINE_INPUT} ${NUMBER_ROOM_INLINE}` : `${inputClass()} ${NUMBER_ROOM}`} ${
        align === 'right' ? 'text-right' : ''
      }`}
    />
  );
...
export function Stat({
  ...
  return (
    <div>
      ...
      {sub && <div className="mt-1 break-words text-xs leading-relaxed text-slate-500">{sub}</div>}
    </div>
  );
}
