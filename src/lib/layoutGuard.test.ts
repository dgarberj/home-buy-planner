...
describe('findUnanchoredAbsolutes', () => {
  ...
  it('accepts inset and logical properties', () => {
    expect(findUnanchoredAbsolutes(`<div className="absolute inset-0" />`)).toEqual([]);
    expect(findUnanchoredAbsolutes(`<div className="absolute start-2" />`)).toEqual([]);
    expect(findUnanchoredAbsolutes(`<div className="absolute end-2" />`)).toEqual([]);
  });
  ...
  it('handles an anchor chosen inside a template literal on the same line', () => {
    const good = '<span className={`absolute top-1/2 ${inline ? \'left-2\' : \'left-3\'}`} />';
    expect(findUnanchoredAbsolutes(good)).toEqual([]);
  });
  ...
  it('reports the line number so the offender is findable', () => {
    const source = ['const a = 1;', '<div className="absolute top-0" />', 'const b = 2;'].join('\n');
  ...
  it('does not fire on unrelated identifiers containing the word', () => {
    expect(findUnanchoredAbsolutes('const absoluteValue = Math.abs(x);')).toEqual([]);
