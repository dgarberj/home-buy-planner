...
const items: BudgetItem[] = [
  ...
  { id: '3', label: 'Rent', category: 'Housing', type: 'fixed', amount: 2000, isRent: true },
  { id: '4', label: 'Car', category: 'Transport', type: 'fixed', amount: 400 },
  { id: '5', label: 'Insurance', category: 'Insurance', type: 'fixed', amount: 200 },
  { id: '6', label: 'Groceries', category: 'Food', type: 'variable', amount: 700 },
  { id: '7', label: 'Fun', category: 'Lifestyle', type: 'variable', amount: 300 },
];
...
describe('deriveBudgetTotals', () => {
  ...
  it('treats a rent-flagged item as rent whatever its type says', () => {
    const odd: BudgetItem[] = [
      { id: 'x', label: 'Rent', category: 'Housing', type: 'variable', amount: 1500, isRent: true },
    ];
  ...
  it('returns zeroes for an empty budget', () => {
    expect(deriveBudgetTotals([])).toEqual({ income: 0, fixed: 0, variable: 0, rent: 0 });
  });
  ...
  it('reports a negative surplus when spending exceeds income', () => {
    const overspent = items.map((i) => (i.type === 'income' ? { ...i, amount: 1000 } : i));
...
describe('deriveStartingBalances', () => {
  const snaps: BalanceSnapshot[] = [
    { id: 'a', date: '2026-01-01', checking: 1000, savings: 1000, investments: 1000, retirement: 1000, debt: 100 },
    { id: 'c', date: '2026-07-01', checking: 5000, savings: 30000, investments: 9000, retirement: 90000, debt: 15000 },
    { id: 'b', date: '2026-04-01', checking: 2000, savings: 2000, investments: 2000, retirement: 2000, debt: 200 },
  ];
...
describe('deriveObligations', () => {
  const dated: BudgetItem[] = [
    { id: 'a', label: 'Support (both)', category: 'Family', type: 'fixed', amount: 1000, endsOn: '2030-09' },
    { id: 'b', label: 'Support (one)', category: 'Family', type: 'fixed', amount: 800, startsOn: '2030-10', endsOn: '2035-09' },
    { id: 'c', label: 'Groceries', category: 'Food', type: 'variable', amount: 700 },
  ];
  ...
  it('runs to the horizon when no end date is given', () => {
    const open: BudgetItem[] = [
      { id: 'x', label: 'Alimony', category: 'Family', type: 'fixed', amount: 500, startsOn: '2027-01' },
    ];
  ...
  it('clamps a commitment that started before the projection did', () => {
    const past: BudgetItem[] = [
      { id: 'x', label: 'Support', category: 'Family', type: 'fixed', amount: 500, startsOn: '2020-01', endsOn: '2030-01' },
    ];
  ...
  it('drops a commitment that ended before the projection did', () => {
    const over: BudgetItem[] = [
      { id: 'x', label: 'Old loan', category: 'Debt', type: 'fixed', amount: 500, endsOn: '2024-01' },
    ];
...
describe('resolveAssumptions', () => {
  ...
  const balances: BalanceSnapshot[] = [
    { id: 'a', date: '2026-07-01', checking: 5000, savings: 30000, investments: 9000, retirement: 90000, debt: 0 },
  ];
  ...
  it('does not mutate the assumptions it was given', () => {
    ...
    resolveAssumptions(base, items, balances, { useBudgetTotals: true, useLatestBalances: true, startDate: '2026-08' });
...
describe('the placeholder seed data is internally consistent', () => {
  ...
  const monthlyAfterContributions =
    budgetSurplus(totals) - dueNow - SEED_ASSUMPTIONS.retirement.employeeMonthly;
  ...
  it('is covered across the year once the January bonus lands', () => {
    const annual = monthlyAfterContributions * 12 + SEED_ASSUMPTIONS.income.annualBonusNet;
  ...
  it('projects far enough ahead to reach every default milestone age', () => {
    ...
    const endAge = SEED_ASSUMPTIONS.household.primaryAge + (DEFAULT_HORIZON_MONTHS - 1) / 12;
