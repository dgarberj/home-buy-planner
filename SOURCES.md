...
*Used by: The county map, the Where to buy table, and every housing cost in the projection.*
...
*Goes stale: Annually — the county publishes a new table each February.*
...
*Goes stale: Annually, effective each 1 July.*
...
*Used by: Affordability ranking, the reach classification, and the waiting analysis.*
...
*Goes stale: Monthly. Prices move; re-check before making an offer.*
...
*Used by: The map tooltip and modal, and the Where to buy table.*
...
*Goes stale: Annually, as new PSSA results publish.*
...
*Used by: The eligibility check, which tests your figures against each programme.*
...
*Goes stale: Check before applying — terms change.*
...
*Goes stale: Funding is limited and reopens periodically — check availability.*
...
*Used by: The PMI calculation and the cash needed at closing.*
...
*Goes stale: Confirm current terms with a lender.*
...
*Goes stale: Get a real quote before committing.*
...
*Used by: Contribution targets, the childcare cost of a second income, and the HSA drawdown options.*
...
*Goes stale: Confirm your employer adopted the higher limit; they are not obliged to.*
...
*Goes stale: Stable rule, but confirm with a tax professional before a large withdrawal.*
...
*Used by: The second income presets and the retirement discussion.*
...
*Goes stale: Stable, but immigration rules change.*
...
*Goes stale: Credit thresholds change annually.*
